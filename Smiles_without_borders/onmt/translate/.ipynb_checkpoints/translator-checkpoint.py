...
class Translator(object):
    ...
    def _fast_translate_batch(self,
        ...
        # Initialize topk_log_probs more directly without triggering warnings
        topk_log_probs = torch.full((batch_size, beam_size), float("-inf"), device=device)
        topk_log_probs[:, 0] = 0.0  # Set the first beam's log prob to 0.0 for each item in the batch

        # Flatten if necessary, depending on how you plan to use topk_log_probs
        topk_log_probs = topk_log_probs.view(-1)
